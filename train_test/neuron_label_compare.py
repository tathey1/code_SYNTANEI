...
class Compare_Segmentation_Label_CUDA_New():
    ...
    def printing(self):
        print(self.classes, self.printer, model = self.model, grid_size = self.grid_size, maximum = self.maximum)

    def printing_(self):
        print(Compare_Segmentation_Label_CUDA_New.classes, self.printer, model = self.model, grid_size = self.grid_size, maximum = self.maximum)
        Compare_Segmentation_Label_CUDA_New.classes = None




def print(classes, printer, model = 'train', grid_size = 36, maximum = 2):
    """
    打印结果
    :return:
    """
